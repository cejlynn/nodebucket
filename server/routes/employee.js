...
router.put("/:empId/tasks", (req, res, next) => {
  try {
    ...
    // connects to mongo, finds collection of employees
    mongo(async (db) => {
      const employee = await db.collection("employees").findOne({ empId });
      console.log("employee", employee);

      // if its not a valid employee, throws an error 404
      if (!employee) {
        const err = new Error("unable to find employee with empId " + empId);
        err.status = 404;
        console.log("err", err);
      ...
      // if its not valid, throws an error 400
      if (!valid) {
        const err = new Error("bad request");
        err.status = 400;
        err.errors = validator.errors;
      ...
      // updates the employee document
      const result = await db
        .collection("employees")
        .updateOne({ empId }, { $set: { todo: tasks.todo, done: tasks.done } });
      console.log("result", result);

      // if no result, throws an error 404
      if (!result.modifiedCount) {
...
router.delete("/:empId/tasks/:taskId", (req, res, next) => {
  ...
  try {
    ...
    // if not a number, throws an error 400
    if (isNaN(empId)) {
      const err = new Error("input must be a number");
      err.status = 400;
      console.log("err", err);
